...
public class SkyIslandTopHeightProvider implements FacetProvider {
    ...
    public void setSeed(long seed) {
        ...
        surfaceNoise2 = new SubSampledNoise(new BrownianNoise(new SimplexNoise(seed + 67), 8), new Vector2f(0.001f,
                0.001f), 1);
    ...
    public void process(GeneratingRegion region) {
        ...
        for (BaseVector2i position : processRegion.contents()) {
            float height = Math.abs(2 + surfaceNoise1.noise(position.x(), position.y()) * 7
                    + surfaceNoise2.noise(position.x(), position.y()) * 7 + surfaceNoise3.noise(position.x(),
                    position.y()) * 5);
