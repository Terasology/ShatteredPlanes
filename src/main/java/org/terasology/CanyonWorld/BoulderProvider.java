...
public class BoulderProvider implements FacetProvider {
    ...
    public void setSeed(long seed) {
        PreNoise=new BrownianNoise(new PerlinNoise(seed + 50), 12);
    ...
    public void process(GeneratingRegion region) {
        ...
        Border3D border = region.getBorderForFacet(BoulderFacet.class);
        BoulderFacet facet = new BoulderFacet(region.getRegion(), border);
        ...
        for (int wz = region.getRegion().minZ(); wz <= region.getRegion().maxZ(); wz++) {
            for (int wx = region.getRegion().minX(); wx <= region.getRegion().maxX(); wx++) {
                ...
                if (surfaceHeight >= region.getRegion().minY() &&
                        surfaceHeight <= region.getRegion().maxY()-15) {

                    for (int wy = surfaceHeight; (wy <= region.getRegion().maxY() && wy <= surfaceHeight+15 && surfaceHeight>0); wy++) {


                        // TODO: check for overlap
                        float noiseVal = TeraMath.clamp(noise.noise(wx, wz, wy)+mountainNoise1.noise(wx,wz,wy)*4, 0, 1);

                        if (noiseVal > 0.35) {
                            facet.setWorld(wx, wy, wz, (float) 1.0);
        ...
        region.setRegionFacet(BoulderFacet.class, facet);
    }
